...
public class AdsReadUtil {
    ...
    public ResultListsEntity dev1StructRead(){
        ...
        try {
            ...
            err = AdsCallDllFunction.adsSyncReadWriteReq(
                    ...
                    symBuff);
            if(err!=0) { System.out.println("Error: Get handle: 0x"
                    + Long.toHexString(err)); }
            ...
            if(err!=0) {
                ...
                for (int i = 0; i<15; i++){
                    ...
                    if (i<8){
                        ...
                        // 处理节点数据
                        for (int j=0;j<45;j++){
                            nodes[j].set(nodeResultEntity,responseBuffer.getFloat(i*45*4+j*4));
                        }
    ...
    public void dev2StructRead(){
        ...
        try {
            AdsCallDllFunction.adsPortOpen();
            err = AdsCallDllFunction.getLocalAddress(addr);
            addr.setNetIdStringEx(NetId2); //耀光计划
            addr.setPort(851);
            if (err != 0) {
                System.out.println("Read Error PLC1: 0x"
                        + Long.toHexString(err));
            ...
            if(err!=0) {
                ...
                for (int i = 0; i<length/4; i++){
                    ByteBuffer responseBuffer = ByteBuffer.wrap(dataBuff.getByteArray());
                    responseBuffer.order(ByteOrder.LITTLE_ENDIAN);
                    System.out.println(responseBuffer.getFloat(i*4));
                }
            }
